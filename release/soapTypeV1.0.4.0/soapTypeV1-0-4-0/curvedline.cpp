...
//        paint->setBrush(QColor(102,102,102));
//        int w = QString::number(i).size()*11 + 10*2;
//        paint->drawRect(QRect(x1 - 10,0,w,LineHight-1));
...
//        int x1 =  (Eachbase.pos)*regionShow_xAxisExpand;
//        paint->drawText(x1,2*LineHight-9,QString(Eachbase.basetype));
//    }
    ...
    for(int i=0,j=0;i<cuv_regionShowData->base[currentfile].count();++i,++j)
    {
        ...
        if(one.Pos+cuv_regionShowData->moves[currentfile]==Eachbase.pos&&one.base!=Eachbase.basetype&&one.base!='L')
        {
            ...
            paint->drawText(x1-7,4*LineHight-9,QString(one.base));
        }
    }
