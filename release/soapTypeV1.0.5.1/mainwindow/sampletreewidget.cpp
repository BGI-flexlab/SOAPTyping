    ...
    :QTreeWidget(parent)
{
    ...
    setStyle(QStyleFactory::create("Windows")); //新增
    setSampleTreeWidgetDefault();
    setSampleTreeWidgetActionAndConnectionDefault();
    this->setFont(QFont("微软雅黑", 8)); //新增
    this->setContentsMargins(10,0,0,0); //新增
}
...
void SampleTreeWidget::setSampleTreeWidgetDefault()
{
    setColumnCount(3);
    header()->hide();
    setSelectionBehavior(QAbstractItemView::SelectRows); //改，SelectItems->SelectRows
...
void SampleTreeWidget::setData(QVector<SampleTreeInfo> &sampleTreeInfoList)
{
    ...
    for(int i=0; i<sampleSize; i++)
    {
        ...
        top->setSizeHint(0,QSize(200,20));//新增
        ...
        for(int j=0; j<fileSize; j++)
        {
            ...
            child->setSizeHint(0,QSize(300,20));//新增
        ...
        for(int j=0; j<gsspSize; j++)
        {
            ...
            {
                //改，删                child->setText(1, QString("%1%2").arg(gsspTreeInfo.exonIndex).arg(gsspTreeInfo.rOrF));
...
QVector<SampleTreeInfo_t> SampleTreeWidget::sortData(QVector<SampleTreeInfo> sampleTreeInfoList)
{
    ...
    for(int i = 0;i < sampleSize;i++)
    {
        ...
        for(int j = 0;j < GsspSize;j++)
        {
            ...
            /*由于样品峰图和Gssp峰图原本就是排序好的，所以一次插入排序即可将GSSP文件插入到适当的位置*/
            bool flag = true;
            for(int u=0; u<tmpSampleTreeInfo.treeinfo.size(); u++){
                if(tmpSampleTreeInfo.treeinfo[u].exonIndex>tmpTreeInfo.exonIndex){
                    tmpSampleTreeInfo.treeinfo.insert(u, tmpTreeInfo);
                    flag = false;
                    break;
                }
            }
...
//            //                }
//            //            }
//            for(int u = j+1;u<treeSize;u++)
//            {
//                if(tmpSampleTreeInfo.treeinfo[j].exonIndex > tmpSampleTreeInfo.treeinfo[u].exonIndex)
//                {
//                    tmpTreeInfo = tmpSampleTreeInfo.treeinfo[j];
//                    tmpSampleTreeInfo.treeinfo[j] = tmpSampleTreeInfo.treeinfo[u];
//                    tmpSampleTreeInfo.treeinfo[u] = tmpTreeInfo;
//                }
//            }
//        }
...
void SampleTreeWidget::emitFileChangedSignal(int topIndex, int childIndex, int type)
{
    ...
    if(item0->text(1).contains("Filter")) //改2->1
    {
    ...
    signalInfo_.rOrF = fileTreeInfo.rOrF;
    emit signalFileChnaged(signalInfo_, type);
    emit signalGeneName(sampleTreeInfo.geneName); //新增
}
...
void SampleTreeWidget::slotFileChanged(SignalInfo &signalInfo, int type)
{
    ...
    if(type!=0)
    {
        ...
        sampleTreeInfoList_.clear();
        m_TreeInfoList.clear(); //新增
        getSampleTreeDataFromRealTimeDatabase(sampleTreeInfoList_);
        m_TreeInfoList = sortData(sampleTreeInfoList_); //新增
...
void SampleTreeWidget::slotItemChanged(QTreeWidgetItem *item0, int col)
{
    ...
    //改    if(col == 1 || item0->text(2).isEmpty())
    //    {
