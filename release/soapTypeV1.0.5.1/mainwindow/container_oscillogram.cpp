...
Container_Oscillogram::Container_Oscillogram(
    ...
    QWidget(parent)
{
//20180608close qDebug("container_oscillogram construct begin");
    ...
    curvedLines.clear();
//20180608close qDebug()<<"container_oscillogram construct filecount "<<filecount;
    for( int i=0 ; i < filecount ; i++ )
    {

//20180608close qDebug()<<"container_oscillogram construct fileIndex "<<i;
...
//20180608close qDebug()<<"container_oscillogram construct fileIndex end "<<i;
        connect(renderArea,SIGNAL(mouserelease(QMouseEvent *)),this,SLOT(mouseRelease(QMouseEvent *)));
        connect(renderArea,SIGNAL(signalClickLetter(int,int)),this,SIGNAL(signalClickLetter(int,int)));//新增
        connect(renderArea,SIGNAL(formbasechanged(char,int,int)),this,SIGNAL(formbasechanged(char,int,int)));//新增
        renderArea->setCurvedLinePos(&curvedLines);//新增
    }
//20180608close qDebug()<<"container_oscillogram construct xxx1 "<<filecount;
    for(int i = 0;i < filecount;i++) //新增
    {
        curvedLines[i]->getAllSoapLetterPos(); //新增
    }
    ...
    int j = 0;
//20180608close qDebug()<<"container_oscillogram construct xxx2 "<<filecount;
    ...
    setthiswidth();
//20180608close qDebug("container_oscillogram construct end");
}


void Container_Oscillogram :: paintEvent ( QPaintEvent *)
{
//20180608close qDebug("container_oscillogram paintEvent begin");
    QPainter paint( this );

    /**新增Start**/
    paint.setPen(QColor(211,211,211));
    paint.setBrush(QColor(255,255,0,150));
    paint.setRenderHint(QPainter::Antialiasing, false);
    int heightNum = height();
    paint.drawRect(QRect(m_FocusBoxPos,0,20,heightNum -1));
    /*新增End*/

    HighLightGivenFile(paint,currenteditfile);
//20180608close qDebug("container_oscillogram paintEvent end");
}
...
void Container_Oscillogram ::mouseReleaseEvent( QMouseEvent* event )//鼠标单击移动对齐框
{
    if( event -> button() == Qt::LeftButton &&!ChangeFormshow)
    {
        ...
        for(int i = 0;i < filecount;i++)
        {
            curvedLines[i]->getAllSoapLetterPos();
        }
        /**新增End**/
    }
    return;
    ...
    try{
        if(!container_savedata->addordecreaseOne)
        {
            ...
            if(container_savedata->filesyExpand<0&&num<0
                    ||container_savedata->filesyExpand >10) //新增
            {
                                             ...
                                             int focusCurvePos/*鼠标点击距离左边边框的距离*/ )
{
    ...
    m_FocusBoxPos = focusAb1Pos*curvedLines[currenteditfile]->getregionShow_xAxisExpand() - 12; //新增
...
void Container_Oscillogram :: ab1PosToExonPos(int &MoseClickPos , int &exonPos )//根据鼠标位置找到ba1文件中对应的碱基
{
    try{
        ...
        double FocusMarkxPos2 = MoseClickPos/curvedLines[i]->getregionShow_xAxisExpand();
                                                 ...
                                                 int &maxCorrespondingBasePos )
{
    try
    {
        ...
        if(exonPos<0)
        {
//20180608close qDebug() << "The exonPos is not in the current exon!";
            return;
        }
...
//20180608close qDebug()<<"error by data in  alignPosByExonPos";
                continue;
            }
...
//20180608close qDebug()<<"         match pos"<<tempeakchange[m].matchPos<<tempeak[n].matchPos;
