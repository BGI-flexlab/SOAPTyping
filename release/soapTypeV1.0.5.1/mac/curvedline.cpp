...
void CurvedLine::drawAll()
{
    ...
    try{
        ...
        for(int i=0;i<cur_changeinfo.count();++i)
        {
            ...
            {
                //            qDebug()<<cur_changeinfo.count();
                //            qDebug()<<cur_baseinfor.count();
                //            qDebug()<<cur_changeinfo[0].Consen_Pos;
...
void CurvedLine::analysisDataforpeak()
{
    ...
    for(int i=0;i<hypeakVector.count();++i)
    {
        ...
        if(hypeakVector[i].startindex+1>Peakinfos.count()) {continue; qDebug()<<"star big";}
...
bool CurvedLine ::drawletter(QPainter *paint)//画碱基字母
{
    ...
    foreach(const BaseInfor & Eachbase,cur_baseinfor)
    {
        ...
        //        paint->drawText((Eachbase.pos)*regionShow_xAxisExpand+16
        //                        ,2*LineHight-2,QString::number(Eachbase.pos));
        //        paint->drawLine(Eachbase.pos*regionShow_xAxisExpand,0,
        //                        Eachbase.pos*regionShow_xAxisExpand,CurvedHight);
    }
    ...
    for(int i=0,j=0;i<cuv_regionShowData->base[currentfile].count();++i,++j)
    {
        ...
        {
            if(one.Pos+cuv_regionShowData->moves[currentfile]==cuv_regionShowData->base[currentfile][i].pos) {break;qDebug()<<"       break i";}
        }
        ...
        if(one.Pos+cuv_regionShowData->moves[currentfile]==Eachbase.pos&&one.base!=Eachbase.basetype&&one.base!='L')
        {
            ...
            paint->drawText(x1,4*LineHight-2,QString(one.base));
        }
    ...
    foreach(const Base_EditInfo & one,cur_changeinfo)
    {
        ...
        paint->drawText(one.Consen_Pos*regionShow_xAxisExpand,
                        3*LineHight-2,QString(one.base_type));
    }
