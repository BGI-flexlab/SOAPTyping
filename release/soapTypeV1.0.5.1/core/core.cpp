...
bool get_info_from_ab1(FileTable &filetable)
{
    ...
    /**新增Start**/
    double *ratios = (double*) malloc(sizeof(double)*filetable.baseNumber);
    memset(ratios,0,sizeof(double)*filetable.baseNumber);
    double avgsignal =ab1.signalRatio(baseG, baseA, baseT, baseC, base, ratios, base_positions, filetable.baseNumber);
    filetable.avgsignal = avgsignal;
    /**新增End**/
    ...
    /**新增Start**/
    QStringList pathlist = filetable.filePath.split("_");
    if(pathlist[2] == "S117")
    {
        file_align_new.exclude_right_num =abs(strlen(file_align_new.raw_seq) - 270);
    }
    /**新增End**/
    //    if(pathlist[2] == "S50")
    //    {
    //        file_align_new.exclude_left_num = 200;
    //        file_align_new.exclude_right_num =abs(strlen(file_align_new.raw_seq) - 100);
    //    }
