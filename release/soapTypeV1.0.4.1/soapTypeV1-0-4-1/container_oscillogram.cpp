    ...
    QWidget(parent)
{
    ...
    for( int i=0 ; i < filecount ; i++ )
    {
        ...
        connect(renderArea,SIGNAL(signalClickLetter(int,int)),this,SIGNAL(signalClickLetter(int,int)));
        connect(renderArea,SIGNAL(formbasechanged(char,int,int)),this,SIGNAL(formbasechanged(char,int,int)));
        renderArea->setCurvedLinePos(&curvedLines);
    }
    for(int i = 0;i < filecount;i++)
    {
    ...
    try{
        if(!container_savedata->addordecreaseOne)
        {
            ...
            if(container_savedata->filesyExpand<0&&num<0
                    ||container_savedata->filesyExpand >10)
            {
                                              ...
                                              int focusCurvePos/*鼠标点击距离左边边框的距离*/ )
{
    ...
    m_FocusBoxPos = focusAb1Pos*curvedLines[currenteditfile]->getregionShow_xAxisExpand() - 12;
