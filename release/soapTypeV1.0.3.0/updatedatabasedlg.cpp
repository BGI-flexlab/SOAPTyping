...
void UpdateDlg::setDefaultData()
{
    ...
    waitBt1 = new QPushButton("It is Processingg, Please Wait ...", this);
    waitBt1->setHidden(true);
    waitBt1->setMinimumHeight(150);
    waitBt2 = new QPushButton("It is Processingg, Please Wait ...", this);
...
void UpdateDlg::slotClickT2ApplyBt()
{
    ...
    if(geneFile.isEmpty() && oldGeneFile.isEmpty() && alleleFile.isEmpty() &&
            gsspFile.isEmpty() && labAlignFile.isEmpty() && exonTrimFile.isEmpty())
    {
        showWarningBox(QString("Please set one file at least!"));
        return;
    }

    if(isFileExists(geneFile)<0 || isFileExists(oldGeneFile)<0 || isFileExists(alleleFile)<0
            || isFileExists(gsspFile)<0 || isFileExists(labAlignFile)<0 || isFileExists(exonTrimFile)<0)
    {
        return;
    }
