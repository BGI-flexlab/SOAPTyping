...
bool log_init(LogLevel l, const char* p_modulename, const char* p_logdir)
{
    ...
    snprintf(_location_str, _LOG_PATH_LEN, "%s/%s-%04d%02d%02d%02d%02d%02d.log", p_logdir, p_modulename,
...
int Log_Writer::premakestr(char* m_buffer, LogLevel l)
{
	...
	now = time(&now);;
    struct tm *vtm = localtime(&now);
    return snprintf(m_buffer, _LOG_BUFFSIZE, "%s: %02d:%02d:%02d ", logLevelToString(l),
