...
//    setStyleSheet("QToolBar{background-color:#cedcc5;border: 0px solid grey;height:140px};QToolBar::QAction:hover{color:blue};");
    ...
    exonNavigator->setMinimumHeight(80);
//    exonNavigator->resize(100,100);
    baseAlignTable->resize(100,100);
    sampleTree->setObjectName("sampleTree");
    baseAlignTable->setObjectName("baseAlignTable");
    matchListTable->setObjectName("matchListTable");
    ...
    rightSplitter->setStretchFactor(0,59);
    rightSplitter->setStretchFactor(1,100);
    ...
    vBoxLayout->setContentsMargins(0,0,8,5);
    ...
    connect(sampleTree, SIGNAL(signalGeneName(QString)), align, SLOT(slotGetGeneName(QString)));
    ...
    connect(main_mscillogram, SIGNAL(signalAB1Information(QString)), this, SLOT(slotMsgStr(QString)));
...
//    menuBar->setMinimumHeight(100);
    ...
    StatusBarWidget *atgcn = new StatusBarWidget;
    statusBar->setContentsMargins(7,0,0,0);
    atgcn->setMinimumHeight(40);
    statusBar->addWidget(atgcn);
    msgLabel = new QLineEdit;
    msgLabel->setObjectName("msgLabel");
    msgLabel->setEnabled(false);
    msgLabel->setText("Ready");
    msgLabel->setMinimumHeight(40);
    msgLabel->setMinimumWidth(872);
    msgLabel->setMinimumWidth(872);
...
void MainWindow::slotAlignPair()
{
    ...
    QStringList typeResultList = matchListTable->getTypeResult();
    QStringList typeResult;

    AllelePairDlg align;
    QString allele1, allele2;
    align.setData(signalInfo_.geneName, &allele1, &allele2);
    if(typeResultList.size() != 0)
    {
        typeResult = typeResultList[0].split(",");
        align.findComboxItem(typeResult[1],typeResult[2]);
    }
    align.exec();
