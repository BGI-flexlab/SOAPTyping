...
bool loadRealTimeDatabase()
{

    QFile file(realTimeDatabaseFilePath);
    if(!file.exists())
    {
        return false;
    }
...
void insertOneFileTable(const FileTable &fileTable)
{
    ...
    query.bindValue(30, fileTable.avgsignal); //新增
    bool isSuccess = query.exec();
    if(!isSuccess)
    {
...
void modifySequence(QByteArray &sequence, QSet<int> &editPostion, int exonStartPos, int excludeLeft, int excludeRight, const QString &editString)
{
    if(editString.size()>0)
    {
        ...
        for(int i=0; i<edits.size(); i++)
        {
            ...
            int pos = posBase.at(0).toInt(); //脥芒脧脭脳脫脦禄碌茫
...
{
    QSqlQuery query(realTimeDatabase);
    query.prepare("SELECT fileName,exonIndex,rOrF,isGood,alignResult FROM fileTable WHERE sampleName=? ORDER BY exonIndex DESC,rOrF DESC, fileName DESC");
    query.bindValue(0, sampleName.toAscii());
    bool isSuccess = query.exec();
    if(!isSuccess)
    {
...
{
    QSqlQuery query(realTimeDatabase);
    query.prepare("SELECT fileName,gsspName,exonIndex,rOrF,isGood,alignResult FROM gsspFileTable WHERE sampleName=? ORDER BY exonIndex DESC, rOrF DESC, fileName DESC");
    query.bindValue(0, sampleName.toAscii());
    bool isSuccess = query.exec();
    if(!isSuccess)
    {
...
QList<FileInfo_ly> OgetAllData(QString sampleName ,int exonIndex)
{
    ...
    if(query.exec())
    {
        while(query.next())
        {
            ...
            one.avgsignal=query.value(30).toDouble(); //新增
            one.isgssp=false;
