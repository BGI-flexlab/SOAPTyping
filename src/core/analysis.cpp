...
bool analysisSample(const QByteArray &sampleName)
{
    ...
    if(fileInfos.size()>0)
    {
        ...
        result = comparePatternWithAllele(sampleInfo.patternSequence.data(), sampleInfo.consensusSequence.data(), sampleInfo.geneName, sampleInfo.exonStartPos, sampleInfo.minExonIndex, sampleInfo.maxExonIndex, typeResult, shieldAlleles);
...
int comparePatternWithAllele(char *patternSeq,  char *consensusSeq, const QByteArray &geneName, int exonStartPos, int minExonIndex, int maxExonIndex, QMap<int, QString> &typeResult, QSet<QString> &sheildAlleles)
{
...
int comparePatternWithAlleleByThread(char *patternSeq, QVector<AlleleInfo> &alleleInfos, QVector< QVector<int> > &differentPosInfos, QMap<int, QString> &typeResult)
{
    ...
    if(ThreadNum > 1)
    {
        ...
        //for(int i=ThreadNum-1; i>=0; i--)
        {
            ...
            {
//20180608close qDebug()<<i<<";;;"<<it.key()<<";;;"<<it.value();
    ...
    else
    {
        ...
        {
//20180608close qDebug()<<it.key()<<";;;"<<it.value();;
            typeResult.insertMulti(it.key(), it.value());
            if(it.key()==0)
            {
                isFull = true;
...
/**
  *增加参数，排除兼并序列>一致序列的点
  */
void removeAlleleInfoByPattern(char *patternSeq, QVector<AlleleInfo> &alleleInfos, char *consensusSeq)
{
    QVector<int> pos;
    int minSize = strlen(patternSeq)<strlen(consensusSeq)?strlen(patternSeq):strlen(consensusSeq);
...
void changeAlignMapToAlignString(QMap<int, QString> &typeResult, QString &result)
{
    ...
    //    for(QMap<int, QString>::iterator it=typeResult.begin(); it!=typeResult.end(); it++)
    //    {
    //        QString txt = QString("%1,%2;").arg(it.key()).arg(it.value());
    //        fprintf(fw,"%s\n",txt.toAscii().data());
