...
void MatchListWidget::setMatchListWidgetDefault()
{
    ...
    this->horizontalHeader()->setFixedHeight(40); //新增，设置表头的高度
    this->horizontalHeader()->setDefaultAlignment(Qt::AlignLeft|Qt::AlignVCenter); //新增
    this->setShowGrid(false); //新增，设置不显示格子线
    //    this->setStyleSheet("selection-background-color:#3CBADA"); //改，删
    this->setContentsMargins(8,0,0,0); //新增，
    this->setRowCount(rowNum_);
    this->setColumnCount(colNum_);
    for(int i=0; i<rowNum_; i++)
    {
        this->setRowHeight(i,25); //改15->25
    }
...
void MatchListWidget::setData(QStringList &typeResult)
{
    ...
    QStringList head;
    //改head<<"Mis"<<"Allele1"<<"Allele2"<<"Info"<<"Gssp";
    ...
    if(signalInfo_.isGssp && !signalInfo_.isGsspFilter)
    {
        for(int i=0; i<typeResult.size(); i++)
        {
            ...
            //改this->item(i, 0)->setText(line.at(0));
    ...
    else if(isShieldIndel_)
    {
        for(int i=0; i<typeResult.size(); i++)
        {
            ...
            //改this->item(rowNumTmp_, 0)->setText(line.at(0));
...
void MatchListWidget::slotRowChanged(QTableWidgetItem *itemNow)
{
    ...
    for(int i = 0;i < rowNum_;i++)
    {
        for(int j = 0;j < colNum_;j++)
        {
            ...
            if(i%2==0)
            {
                item->setBackgroundColor(QColor(235,244,223));
            }
            else
            {
    ...
    for(int i = 0;i < colNum_;i++)
    {
        QTableWidgetItem *item = this->item(m_ListRow,i);
        item->setBackgroundColor(QColor(229,229,229));
    }
    m_ListRow = row;
    //新增End
}
...
{
//20180608close qDebug("matchlist slotFileChanged start");
    ...
    setData(typeResult);
    setGssp();
    m_typeResult.clear(); //新增
    m_typeResult = typeResult; //新增
//20180608close qDebug("matchlist slotFileChanged end");
}
...
void MatchListWidget::slotSetFinalType()
{
    ...
    {
        /**
          *修正SetFinalType列表为空问题
          *将开头空格去掉即可
          *逗比啊，为啥非得在列表前边给我加两个空格？？？
          **/
        if(item(i, 0)->text().trimmed().at(0).toAscii() != '0')
            break;
        QStringList line;
