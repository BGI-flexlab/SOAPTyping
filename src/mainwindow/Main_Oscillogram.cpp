...
{
//20180608close qDebug("create_draw start");
    if(data_is_ready==false)
        return ;

//20180608close qDebug("create_draw line1");
    container_oscillogram = new Container_Oscillogram(
                &regionShowData,
                &regionShowDataBak,
                &befordata,this);
//20180608close qDebug("create_draw line2");
    ...
    iscontainer_create=true;
//20180608close qDebug("create_draw line3");
}
...
void Main_Oscillogram::DisconnectFocusBox()
{
    QString strMsg;
    /***
      *修复这个版本下，如果峰图比对到边界，没有自动切除的时候，数组越界问题
      *奇葩，这算法写的我都不想改了，临时加点判断吧，至少取值的时候，不能越界，所以加了0和maxBase与match+-的判断
      **/
    int maxBase = 0;//修复点击边界点报错的问题
    if(isClickTable)
    {
        if(isFileChange)
        {
        ...
        else
        {
            ...
            int match=0;
            maxBase = regionShowData.base[currentfile].count();
            bool isMatch = false;
            ...
            if(Remainder == 0)
            {
                ...
                else
                {
                    ...
                    Code1 = match-2<0?'*':regionShowData.base[currentfile][match-2].basetype;
                    Code2 = match-1<0?'*':regionShowData.base[currentfile][match-1].basetype;
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(Remainder==1)
                    {
                        ...
                        Code2 = match+1>=maxBase?'*':regionShowData.base[currentfile][match+1].basetype;
                        Code3 = match+2>=maxBase?'*':regionShowData.base[currentfile][match+2].basetype;
                        for(int i =0;i < regionShowData.edit_info[currentfile].count();i++)
                        {
                    ...
                    {
                        QString Code1,Code2,Code3;
                        Code1 = match-1<0?'*':regionShowData.base[currentfile][match-1].basetype;
                        Code2 = regionShowData.base[currentfile][match].basetype;
                        Code3 = match+1>=maxBase?'*':regionShowData.base[currentfile][match+1].basetype;
                        for(int i =0;i < regionShowData.edit_info[currentfile].count();i++)
                        {
...
bool Main_Oscillogram :: getRegionShowData( SignalInfo & fileSignal ,bool getdata)
{
    ...
    try{
        ...
        {
//20180608close qDebug()<<"FileSignalInfo ERROR";
            return false;
        }
    ...
    foreach (const FileInfo_ly &fileInfo,  fileinfo_lyList)
    {
        try
        {
            ...
            {
//20180608close qDebug()<<"The number of baseMatchConsensusPos doesn't equal to baseNum in regionShow getRegionShowData()";
                QMessageBox :: warning( this , tr("baseMatchConsensusPos ERROR") , outInfor );
                break;
            }
            if( basePos.size() != baseNum )
            {
//20180608close qDebug()<< "The number of basePos doesn't equal to baseNum in regionShow getRegionShowData()";
            ...
            {
//20180608close qDebug()<< "The number of baseGSignal doesn't equal to baseGnum in regionShow getRegionShowData ()";
            ...
            {
//20180608close qDebug()<< "The number of baseASignal doesn't equal to baseGnum in regionShow getRegionShowData ()";
            ...
            {
//20180608close qDebug()<< "The number of baseTSignal doesn't equal to baseGnum in regionShow getRegionShowData ()";
            ...
            {
//20180608close qDebug()<< "The number of baseCSignal doesn't equal to baseGnum in regionShow getRegionShowData ()";
...
void Main_Oscillogram :: resetAll()
{
    ...
    if(data_is_ready)
    {
        ...
        iscontainer_create=false;
//20180608close qDebug("main_oscillogram resetAll xx3 create_draw");
        create_draw();
        iscontainer_create=true;
    }
}
...

{
    ...
    if( current_exonNum!=exonNum)
    {
        ...
        if(iscontainer_create)
            container_oscillogram->~Container_Oscillogram();
        iscontainer_create=false;
//20180608close qDebug("main_oscillogram slotFocusMark xx3 create_draw");
        create_draw();
        is_emit = true;
    }
    if(iscontainer_create&&data_is_ready)
...
//20180608close qDebug("main_oscillogram slotFileChanged start");
    mutex.lock();
    if(done==true)
    {
        done=false;
        ...
        getAB1Signal();
//20180608close qDebug("main_oscillogram slotFileChanged xx1");
        ...
        if(iscontainer_create)
            container_oscillogram->~Container_Oscillogram();
        iscontainer_create=false;
//20180608close qDebug("main_oscillogram slotFileChanged xx2");
        if(data_is_ready)
        {
            analysisData();
//20180608close qDebug("main_oscillogram slotFileChanged xx3 create_draw");
            create_draw();
        }
        done=true;
    }
    mutex.unlock();
    isFileChange = true;
//20180608close qDebug("main_oscillogram slotFileChanged xx4");
    slotfilechange(false);
//20180608close qDebug("main_oscillogram slotFileChanged end");
}
